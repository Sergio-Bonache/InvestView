...
const resultado = computed(() => {
  ...
  // Fórmula: FV = P*(1+r)^n + A*(((1+r)^n - 1)/r)
  const fv =
    P * Math.pow(1 + r, n) +
    (A > 0 && r > 0
...
<template>
  <section class="container mt-15 mb-15 px-4 mx-auto">
    ...
    <div class="mt-8 bg-gray-50 dark:bg-gray-800 shadow-md rounded-lg p-6 border border-gray-200 dark:border-gray-700 ">
      <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">Resultados</h3>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
        <div>
          <div class="text-gray-500 dark:text-gray-400 text-sm">Total aportado</div>
        ...
        <div>
          <div class="text-gray-500 dark:text-gray-400 text-sm">Intereses ganados</div>
        ...
        <div>
          <div class="text-gray-500 dark:text-gray-400 text-sm">Capital final</div>
    ...
    <p class="mt-6 text-xs text-gray-500 dark:text-gray-400">
      * El cálculo es orientativo y no tiene en cuenta impuestos, comisiones ni inflación.
    </p>
  </section>
</template>